...
export default function Home() {
  ...
  async function submitBatch() {
    ...
    data.append("application", JSON.stringify(form));
    try {
      setBatchResult(await request("/batch", data, 60000));
    } catch (cause) {
  ...
  return <main>
    ...
    {batchResult ? <section className="results" aria-live="polite"><h2>Batch results</h2><div className="batch-table"><div className="batch-head"><span>File</span><span>Status</span><span>Details</span></div>{batchResult.items.map((item) => <div className="batch-row" key={item.filename}><span>{item.filename}</span>{item.result ? <Status status={item.result.overall_status} /> : <span className="status missing">! Error</span>}<span>{item.error ?? String(item.result?.fields.filter((field) => field.status !== "match").length ?? 0) + " field(s) need attention"}</span></div>)}</div></section> : null}
